...
@Component({
    ...
    styleUrls: ['./portfolio.component.scss', '../shared/css/slideshow.scss']
})
export class PortfolioComponent implements AfterViewInit {
    ...
    constructor(private activatedRoute: ActivatedRoute) {
        this.activatedRoute.data.map(data => data.projects).subscribe((resp) => {
            this.projects = resp;
            console.log('Component fetched projects from resolver');
    ...
    public setSliderValues() {
        ...
        this.slideshow__container.nativeElement.setAttribute('style',
            'width: ' + (this.videoSize * this.projects.length) + 'px; ' + 'transform: matrix(1, 0, 0, 1, ' + this.sliderPosX +
    ...
    public switchVideo(nextVideo: number) {
        ...
        if (this.projects.length !== 1 &&(this.currentVideo === 0 && direction === '-'
            || (0 < this.currentVideo && this.currentVideo < this.projects.length - 1)
            || this.currentVideo === this.projects.length - 1 && direction === '+')
            && !this.animating) {
     ...
     */
    ngAfterViewInit() {
        console.log('Portfolio AfterViewInit');
        this.onResize(event);
